...
if check_date:
    ...
    if (dia_semana.lower()=="lunes")or(dia_semana.lower()=="martes") or(dia_semana.lower()=="miercoles"):
        ...
        if(check_examen.lower() == "si"):
            ...
            print(f"El porcentaje de aprobados es: {int((cant_aprobados/cant_total)*100)}%")#MOSTRAR PORCENTAJE DE APROBADOS
    elif dia_semana.lower() =="jueves": #DIA JUEVES
        cant_total = int(input("Cantidad de alumnos: "))
        cant_asistencia = int(input("Asistencia: "))
        asistencia_final = print("Asistió la mayoriía")if int((cant_asistencia/cant_total)*100)>=50 else print("No asistió la mayoria")#MOSTRAR ASISTENCIA
    else: #DIA VIERNES 
